...
<title>VulnScope Pro - ZeroDay Security Intelligence Platform</title>
...
<style>
:root{
  --bg-root:#020408;--bg-card:rgba(10,18,30,0.9);
...
/* VULN LIST */
#vulnList{max-height:520px;overflow-y:auto;display:flex;flex-direction:column;gap:9px;padding:16px 18px}
...
<aside id="sb">
  <div class="sb-logo">
    ...
    <div class="sb-logo-txt"><h2>VulnScope <span>Pro</span></h2><p>ZeroDay Security</p></div>
  </div>
  <nav class="sb-nav">
  ...
  <div class="sb-apis">
    ...
    <?php
// API status sidebar rows — injected inline
...
<div class="sb-api-row">
    <div class="sb-api-name">
        <span class="dot <?php echo $_aok ? 'dot-on' : 'dot-off'; ?>"></span>
        <?php echo $_al; ?>
    </div>
  ...
  <header id="topbar">
    <div class="tb-left">
      ...
      <div class="tb-bread">
        ...
        <span class="cr-active">VulnScope Pro</span>
      </div>
    </div>
    <div class="tb-right">
  ...
  <main id="pg">
    <section class="scan-panel">
      <div class="scan-ph">
        ...
        <div class="eng-badge">
          <i class="fas fa-circle" style="font-size:7px;color:#22c55e;animation:dpulse 1.5s infinite"></i>
          Scan Core Active
        </div>
      </div>
      <form id="scanForm">
      ...
      <div id="loader">
        ...
        <div class="ld-bar-wrap">
          <div class="ld-txt"><i class="fas fa-circle-notch fa-spin" style="margin-right:6px"></i>Initializing scan engine &amp; correlating intelligence...</div>
...
(function(){
  ...
  function draw(){
    ...
    for(let i=0;i<pts.length;i++)for(let j=i+1;j<pts.length;j++){
      ...
      if(d<110){ctx.globalAlpha=1-d/110;ctx.beginPath();ctx.moveTo(pts[i].x,pts[i].y);ctx.lineTo(pts[j].x,pts[j].y);ctx.stroke()}
    }
...
function ctr(el,target,dur=900){const s=performance.now(),f=parseInt(el.innerText)||0;(function step(n){const p=Math.min((n-s)/dur,1),e=1-Math.pow(1-p,3);el.innerText=Math.round(f+(target-f)*e);if(p<1)requestAnimationFrame(step)})(s)}
document.getElementById('scanForm').onsubmit=async(e)=>{
  ...
  btn.disabled=true;ldr.classList.add('show');
  ...
  finally{btn.disabled=false;ldr.classList.remove('show')}
};
...
function ico(s){if(!s)return 'fa-server';s=s.toLowerCase();if(s.includes('http')||s.includes('www'))return 'fa-globe';if(s.includes('ssh'))return 'fa-terminal';if(s.includes('sql')||s.includes('mysql')||s.includes('mongo'))return 'fa-database';if(s.includes('ftp'))return 'fa-upload';if(s.includes('smtp')||s.includes('mail')||s.includes('pop')||s.includes('imap'))return 'fa-envelope';if(s.includes('dns')||s.includes('domain'))return 'fa-sitemap';if(s.includes('rdp')||s.includes('vnc'))return 'fa-desktop';if(s.includes('redis'))return 'fa-memory';return 'fa-server';}
function renderPorts(ports){
  document.getElementById('portGrid').innerHTML=ports.length
    ?ports.map((p,i)=>`<div class="svc-card" style="animation-delay:${i*35}ms"><div class="svc-ico"><i class="fas ${ico(p.service)}"></i></div><div class="svc-info"><div class="svc-nm"><span>${(p.service||'unknown').toUpperCase()}</span><span class="open-pill">OPEN</span></div><div class="svc-port">${p.port}/TCP</div><div class="svc-prod">${[p.product,p.version].filter(Boolean).join(' ')||'&mdash;'}</div></div></div>`).join('')
    :`<div class="empty-s" style="grid-column:1/-1"><i class="fas fa-network-wired"></i>No open services detected</div>`;
}
function renderVulns(f){
  const list=document.getElementById('vulnList');
  list.innerHTML=f.length
    ?f.map((v,i)=>`<div class="vc severity-${v.severity||'Info'}" style="animation-delay:${i*30}ms"><div class="vc-top"><div class="vc-ids"><span class="vc-id">${v.id}</span><span class="src-badge">${v.source}</span></div><div class="sev-tags"><span class="sp ${v.severity||'Info'}">${v.severity||'Info'}</span><span class="cvss-chip">CVSS ${v.cvss>0?v.cvss:'N/A'}</span></div></div><p class="vc-sum">${v.summary}</p><div class="vc-meta"><div class="vc-svc"><i class="fas fa-microchip"></i>${v.affected_service||'&mdash;'}</div><span class="port-chip">PORT ${v.port}</span></div></div>`).join('')
    :`<div class="empty-s"><i class="fas fa-shield-check"></i>No CVE matches &mdash; target appears hardened.</div>`;
}
function fvulns(sev,btn){document.querySelectorAll('.fb').forEach(b=>b.classList.remove('active'));btn.classList.add('active');renderVulns(sev==='all'?_all:_all.filter(f=>f.severity===sev));}
function exportReport(){
  if(!_last)return showError('No scan data yet.');
